...
void send_to_client(int socket, const std::string& msg) {
    if (socket != -1) send(socket, msg.c_str(), msg.size(), 0);
}
...
void broadcast_all(const std::string& message) {
    std::vector<int> sockets;
    {
        ...
        for (auto& p : clients) sockets.push_back(p.second.socket);
    }
    for (int s : sockets) send_to_client(s, message);
}
...
bool send_private(const std::string& target, const std::string& msg, int sender_socket, const std::string& sender_name) {
    std::lock_guard<std::mutex> lock(clients_mutex);
...
bool has_command(const std::string& role, const std::string& cmd) {
    auto it = role_commands.find(role);
    if (it == role_commands.end()) return false;
    return std::find(it->second.begin(), it->second.end(), cmd) != it->second.end();
}
...
void change_role(const std::string& target, const std::string& new_role, int sender_socket, const std::string& sender_name) {
    {
        std::lock_guard<std::mutex> lock(db_mutex);
        auto it = user_db.find(target);
        if (it == user_db.end()) {
            send_to_client(sender_socket, "User not found.\n");
            return;
        }
        if (target == sender_name) {
            send_to_client(sender_socket, "You cannot change your own role.\n");
            return;
        }
        if (it->second.role == "Admin" && new_role != "Admin") {
            send_to_client(sender_socket, "You cannot demote another Admin.\n");
            return;
        }
    ...
    std::string msg = target + "'s role is now \"" + new_role + "\"";
    std::cout << msg << std::endl;
    broadcast(msg + "\n", -1);
...
void handle_client(int client_socket) {
    ...
    while (running) {
        ...
        if (message.substr(0, 5) == "/msg ") {
        ...
        else if (message.substr(0, 10) == "/broadcast " && has_command(role, "/broadcast")) {
        ...
        else if (message.substr(0, 5) == "/kick" && has_command(role, "/kick")) {
        ...
        else if (message.substr(0, 4) == "/ban" && has_command(role, "/ban")) {
        ...
        else if (message.substr(0, 7) == "/unban" && has_command(role, "/unban")) {
        ...
        else if (message.substr(0, 5) == "/mute" && has_command(role, "/mute")) {
            std::string rest = message.substr(6);
            size_t sp1 = rest.find(' ');
            if (sp1 == std::string::npos) {
        ...
        else if (message.substr(0, 7) == "/unmute" && has_command(role, "/unmute")) {
            std::string target = message.substr(8);
        ...
        else if (message.substr(0, 3) == "/cr" && has_command(role, "/cr")) {
            ...
            change_role(target, new_role, client_socket, username);
        }
...
void console_input() {
    ...
    while (running) {
        ...
        std::cout << "\r\033[KServer message sent.\n";
        std::cout << "Server [SERVER]> " << std::flush;
    }
}
...
int main() {
    ...
    while (running) {
        ...
        if (FD_ISSET(server_fd, &readfds)) {
            ...
            std::cout << "\r\033[KNew client connected.\n";
            std::cout << "Server [SERVER]> " << std::flush;
        }
    }
